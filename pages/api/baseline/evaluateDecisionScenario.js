...
async function updateUserProgress(userId, difficulty, level, stars , score,
  questionsAttempted) {
  try {
    ...
    const apiUrl = `${process.env.API_URL || 'http://localhost:3000'}/api/baseline/updatePracticeProgress`;
...
const handler = async (req, res) => {
  ...
  try {
   ...
   const score = evaluation?.overallEvaluation?.score || 0;
const stars = evaluation?.overallEvaluation?.stars || 0;
    await updateUserProgress(userId, difficulty, level, stars ,score,
  responses.length);
