...
const [selectedPlatform, setSelectedPlatform] = useState(platforms[0]);
  ...
  const storeScore = async (jobRole, email, overallScore) => {
    try {
      const collageName = user?.collageName || "Unknown Collage";
      await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/baseline/overallScore`, {
        method: 'POST',
  ...
  const storeReport = async (jobRole, email, reportAnalysis) => {
    try {
      const collageName = user?.collageName || 'Unknown College';
      await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/baseline/saveAndGetReport`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  useEffect(() => {
    ...
    const fetchJobRole = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/baseline/getReadyQuestionsAndAnswers?jobRoleId=${jobRoleId}`);
        localStorage.setItem('status', "processing");
        ...
        await storeScore(data.data.role, data.data.email, overallScore);
        await storeReport(data.data.role, data.data.email, reportText);
       // Update Step 2 only for Resume Based reports
if (data.data.role === "Resume Based") {
  const normalizedResumeScore = Math.round((overallScore / 50) * 100);
  await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/baseline/stepProgress`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      userId: user?.id,
      stepKey: "step2_resume",
      score: normalizedResumeScore,
    }),
  });
}else {
  // =========================
  ...
  // =========================
  const normalizedMockScore = Math.max(
    0,
    Math.min(
      100,
  ...
  await fetch(
    `${process.env.NEXT_PUBLIC_HOST}/api/baseline/stepProgress`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      ...
      body: JSON.stringify({
        userId: user?.id ,
        stepKey: "step6_mockInterview",
        score: normalizedMockScore,
      }),
    }
  );
}
