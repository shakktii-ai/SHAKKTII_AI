...
function SpeakingPractice() {
  ...
  const fetchLevelProgress = async () => {
    ...
    try {
      ...
      try {
        const response = await fetch(`/api/baseline/getPracticeProgress?skillArea=Speaking&difficulty=${difficulty}&userId=${userId}`, {
          method: 'GET',
          headers: {
  ...
  const submitForFeedback = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/baseline/submitPracticeResponse', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
  ...
  const evaluateLevelCompletion = async () => {
    try {
      ...
      const response = await fetch('/api/baseline/evaluateLevelCompletion', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      try {
        if (response.ok) {
          const result = await response.json();
           // Mark Listening as completed in Step Progress
  await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/baseline/stepProgress`, {
    ...
    body: JSON.stringify({
      userId,
      stepKey: "step4_softSkills",
      subTestKey: "speaking",
      score: Math.round(((result?.levelProgress?.stars || 0) / 3) * 100),
    }),
  });
      ...
      <div className="min-h-screen bg-gray-100" style={{ backgroundImage: "url('/BG.jpg')", backgroundSize: 'cover', backgroundPosition: 'center', position: 'relative', backgroundColor: 'rgba(0,0,0,0.7)', backgroundBlendMode: 'overlay' }}>
        <div className="container mx-auto px-4 py-16">
          ...
          <div className="flex items-center justify-between mb-12">
            <div>
              <button 
                onClick={() => router.push('/BaseLine/skills')} 
