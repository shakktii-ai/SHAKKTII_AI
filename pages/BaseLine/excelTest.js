...
function ExcelTest() {
  ...
  const fetchQuestions = useCallback(async () => {
    try {
      const res = await fetch("/api/baseline/excelTest/questions");
  ...
  const evaluateTest = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/baseline/excelTest/evaluate?userId=${encodeURIComponent(userId)}`,
        {
          method: "POST",
      ...
      // Mark Excel Assessment as completed
      await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/baseline/stepProgress`, {
        ...
        body: JSON.stringify({
          userId,
          stepKey: "step5_techSkills",
          subTestKey: "excelAssessment",
      ...
      <div className="min-h-screen bg-gray-100 flex items-center justify-center p-6">
        <div className="bg-white p-6 rounded-xl shadow-md max-w-2xl w-full text-center">
          ...
          <div className="flex justify-center gap-2 mt-4">
            ...
            <button onClick={() => router.push('/BaeLine/techSkills')} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">Back to Practices</button>
          </div>
        </div>
      </div>
    );
