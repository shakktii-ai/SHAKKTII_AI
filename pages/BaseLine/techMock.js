...
export const fetchAssessmentQuestions = async (userDetails) => {
    try {
        const res = await fetch("/api/baseline/techTest", {
            method: "POST",
...
const userEmail = loggedInUser.email;
        ...
        try {
            const res = await fetch('/api/baseline/submit-full-assessment', {
                method: 'POST',
                headers: {
            ...
            if (res.ok) {
                ...
                // Mark Technical MCQ as completed
    await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/baseline/stepProgress`, {
        ...
        body: JSON.stringify({
            ...
            subTestKey: "technicalMcq",
            score: result?.scores?.technical ?? 0,
        }),
    });
                setStage('success');
            } else {
