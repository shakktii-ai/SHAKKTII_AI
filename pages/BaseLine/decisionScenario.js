...
function DecisionScenario() {
  ...
  const fetchLevelProgress = async () => {
    try {
      ...
      const url = `/api/baseline/getPracticeProgress?userId=${encodeURIComponent(userId)}&skillArea=DecisionMaking&_t=${Date.now()}`;
  ...
  const evaluateResponses = async () => {
    try {
      ...
      const response = await fetch('/api/baseline/evaluateDecisionScenario', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const result = await response.json();
      ...
      if (response.ok) {
        await fetch('/api/baseline/stepProgress', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            userId,
            stepKey: 'step3_decision',
            score: result?.score || result?.evaluation?.overallEvaluation?.score || 0,
          })
        });
      }
