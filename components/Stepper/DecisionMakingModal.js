...
export default function DecisionMakingModal({
  isOpen = true,
  onClose,
  onUploadResume,
  status = 'Not Started',
...
const router = useRouter();
  const tags = [
    ...
    'Reccommended Practice Plans',
   
  ];
 const handleRedirect = () => {
    if (onClose) onClose();
    router.push('/BaseLine/decisionScenario');
  };
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 p-3 sm:p-4 overflow-y-auto no-scrollbar">
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden max-h-[90vh] flex flex-col my-auto border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="bg-[#DC2E7C] text-white p-5 sm:p-6 relative flex-shrink-0">
          {/* Close Button */}
          <button
            onClick={onClose}
          ...
          <div className="flex items-start space-x-3.5 mb-3">
            ...
            <div className="w-14 h-14 rounded-2xl bg-[#B0C9FF]/40 flex items-center justify-center overflow-hidden flex-shrink-0">
              <img
                src="/step3img.svg" // Replace with your asset path
                alt="Brain Icon"
        ...
        <div className="p-5 sm:p-6 overflow-y-auto no-scrollbar space-y-6 flex-1 text-slate-800">
          ...
          <div>
            ...
            <div className="bg-[#F4F4F4] rounded-xl p-4 text-xs text- leading-relaxed border border-black/16">
             Practice real-world scenarios to improve your problem-solving, critical thinking, and decision-making skills.
            </div>
          </div>
        ...
        <div className="px-5 py-4 border-t border-slate-100 bg-white flex-shrink-0 text-center space-y-2">
          <button
            ...
            className="w-full py-3.5 px-4 bg-[#DC2E7C] active:scale-[0.99] text-white rounded-2xl font-semibold text-sm transition-all shadow-md  flex items-center justify-center space-x-2"
          >
